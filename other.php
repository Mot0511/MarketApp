...
<div class="content">
	<div class="newapps">
	<center><h2>Новинки других приложений</h2></center>
	<?php $apprate = array() ?>
	...
	<!-- <?php
	$cfr = count(scandir('rates/games/'));
	$cfr = $cfr - 2;
	$hr = array();
	$rate = array_combine($hr, $apprate);
	arsort($rate);
	array_unshift($rate, ' ');
	...
	for ($i = 0; $i < $cfr; $i++){
		next($rate);
		array_push($ha, key($rate));
		echo '<script type="text/javascript">
		var i = 0;
		var appdiv = document.getElementById("'.$ha[$i].'").innerHTML;
		var teg = "<div class=\'appdiv\'>";
		var teg2 = "</div>";
		$(".rec").append(teg + appdiv + teg2);
		</script>';
	};
	array_unshift($ha, ' ');
